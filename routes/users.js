...
router.get ('/cart', verifyLogin, async (req, res) => {
  ...
  if (cartCount > 0) {
    ...
    const totalAmount = await userHelpers.totalAmount (
    ...
    res.render ('user/shop/emptyCart', {user});
  }
});
...
router.get ('/order-success', verifyLogin, async (req, res) => {
  const orderItems = await userHelpers.getOrderProducts (
    req.session.userDetails._id
  );
...
router.get ('/my-order',verifyLogin, async (req, res) => {
  ...
  userHelpers.getAllOrders(req.session.userDetails._id).then((response)=>{
    ...
    orders.forEach(element => {
      element.ordered_on = moment(element.ordered_on).format('MM DD YY')
    });
    console.log(orders);
...
router.get('/viewOrderProducts/:id',(req,res)=>{
  ...
  userHelpers.getOrderProducts(req.params.id).then((response)=>{
    ...
    res.render('user/shop/viewOrderProducts',{order,user:req.session.userDetails})
  })
})
...
router.get('/viewOrderDetails',async(req,res)=>{
  userHelpers.getOrderProducts(req.session.orderId).then((response)=>{
    ...
    const ordered_on = moment(orderProducts.ordered_on).format('MM DD YY')
